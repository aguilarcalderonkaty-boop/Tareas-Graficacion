...
def dibujar_cilindro(radius, height=0.2):
    """Dibuja un cilindro usando gluCylinder de GLU"""
    quadric = gluNewQuadric()
    gluCylinder(quadric, radius, radius, height, 30, 30)

def draw_kidneys():
    """Dibuja dos riñones en forma de C enfrentados con tubos internos"""
    glPushMatrix()
    glColor3f(0.9, 0.3, 0.3)  # color rojizo 
    
    # Riñón izquierdo 
   
    
    glPushMatrix()
    glTranslatef(-1.0 + 0.4, 0.4 , 0)
    draw_sphere(0.35, 30, 30)
    glPopMatrix()
    
    glPushMatrix()
    glTranslatef(-1.0 + 0.2, 0.4 - 0.3, 0)
    draw_sphere(0.35, 30, 30)
    glPopMatrix()
    
    
    glPushMatrix()
    glTranslatef(-1.0 + 0.4, 0.4 - 0.3*2, 0)
    draw_sphere(0.35, 30, 30)
    glPopMatrix()
    ...
    glPushMatrix()
       ...
       # Tubo izquierdo
    glColor3f(0.85, 0.67, 0.65)  # color piel 
    glTranslatef(-0.4, 0, 0)   
    glRotatef(90, 0, 1, 0)
    dibujar_cilindro(0.1, 0.3)
    glPopMatrix()
